...
as begin
	...
	for select ConflictID, NameConflict
		from CONFLICT
		order by DateStarted

		open cursor_typeconflict
		fetch next from cursor_typeconflict into @idConflict, @nameConflict
		while @@FETCH_STATUS = 0
			begin
				set @date=(select DateStarted from CONFLICT where ConflictID=@idConflict)
				...
				where ConflictID = @idConflict
					...
					if @list<>''
						begin
							 print char(9)+'Religion: '+@list
							 set @list=''
						end

				select @list=@list +Raw_Mat+CHAR(10)+space(14)
				from ECONOMIC
				where ConflictID=@idConflict
					if @list<>''
						begin
							   print char(9)+'Economic: '+@list
							   set @list=''
						end

				select @list=@list +Region+CHAR(10)+space(17)
				from TERRITORIAL
				where ConflictID=@idConflict
					if @list<>''
						begin
							print char(9)+'Territorial: '+@list
							set @list=''
						end
