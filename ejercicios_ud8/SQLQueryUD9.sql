...
--3 
create or alter trigger VEHICLE_SERVICE
on KILOMETERS
after update
as
begin
    declare @idvehicle int = (select IDVehicle from inserted)
    declare @totalkm int = (select TotalKm from inserted)

    -- Si llega a 40.000 km o más → ON SALE
    if @totalkm >= 40000
    begin
        update VEHICLES
        set StatusVeh = 'On Sale'
        where IDVehicle = @idvehicle
    end
    -- Si llega a 10.000, 20.000 o 30.000 → alerta + INACTIVE
    else if @totalkm >= 10000 and @totalkm < 40000
    begin
        -- Comprobar si es uno de los umbrales de servicio
        if (@totalkm >= 10000 and @totalkm < 20000)
            or (@totalkm >= 20000 and @totalkm < 30000)
            or (@totalkm >= 30000 and @totalkm < 40000)
        begin
            insert into ALERTS_MAINTENANCE (IDVehicle, DateAlert)
            values (@idvehicle, GETDATE())

            update VEHICLES
            set StatusVeh = 'Inactive'
            where IDVehicle = @idvehicle
        end
    end
end
-- 4
create or alter trigger FINISH_SERVICE
on MAINTENANCE
after insert
as
begin
    declare @idvehicle int = (select IDVehicle from inserted)
    declare @maintype varchar(50) = (select MainType from inserted)
    declare @datemaint date = (select DateMaint from inserted)

    -- Solo actúa si es tipo 'Service'
    if @maintype = 'Service'
    begin
        -- Actualizar la alerta más reciente sin fecha de mantenimiento (la pendiente)
        update ALERTS_MAINTENANCE
        set DateMaintenance = @datemaint
        where IDVehicle = @idvehicle
          and DateMaintenance is null

        -- Cambiar estado del vehículo a Available
        update VEHICLES
        set StatusVeh = 'Available'
        where IDVehicle = @idvehicle
    end
